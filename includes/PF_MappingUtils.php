...
class PFMappingUtils {
	...
	public static function getLabelsForTitles(
		...
		foreach ( $values as $k => $value ) {
			...
			if ( $doReverseLookup ) {
				// Labels generated by this extension use the
				// 'displaytitle [pagetitle]' format. Square brackets are
				// not legal in MediaWiki page titles, so a bracketed
				// suffix unambiguously holds the real page title, even
				// if the display title itself contains parentheses or
				// brackets (the page title is always appended last).
				// Values without a bracketed suffix are page titles and
				// are used as is.
				if ( preg_match( '/\[([^\][]*)\]\s*$/', $value, $bracketMatch ) ) {
					$realPage = $bracketMatch[1];
					$innerTitle = Title::newFromText( $realPage );
					// Only use it if the page exists; otherwise leave
					// the value as is.
					if ( $innerTitle instanceof Title && $innerTitle->exists() ) {
						$value = $realPage;
					}
				}
			}
		...
		foreach ( $pageNamesForValues as $value => $pageName ) {
			if ( isset( $allDisplayTitles[ $pageName ] )
				...
				// Square brackets are used (rather than parentheses)
				// because they cannot occur in a real page title, which
				// keeps this label format unambiguously parseable.
	 ...
	 * labels, if there are any, add on the real value, in square
	 * brackets, to all of them.
	 *
	 * @param array $labels
	 * @return array
	 */
